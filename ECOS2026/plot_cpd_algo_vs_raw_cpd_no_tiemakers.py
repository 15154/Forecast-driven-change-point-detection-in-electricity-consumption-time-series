#!/usr/bin/env python3
"""Plot mirrored win/loss bars by CPD algorithm (excluding tiemaker algorithms).
...
def plot_mirrored_bars(counts_df: pd.DataFrame, metric: str, output_plot: Path) -> None:
    ...
    # Match the visual bar size used in the forecast script (15in for ~14 bars)
    ref_n_bars = 14
    fig_w = 15.0 * max(1, len(counts_df)) / ref_n_bars
    fig_w = max(6.0, fig_w)
    ...
    ax.set_title(
        f"Forecast-Driven vs Raw CPD by CPD Algorithm\n"
        f"{metric}\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create mirrored bars by CPD algorithm for Forecast-Driven vs raw CPD (excluding tiemakers)"
    )
    parser.add_argument(
        "--metric",
        type=str,
...
def main() -> int:
    ...
    output_dir = (
        ...
        if args.output_dir
        else script_dir / "results" / "plots" / "cpd_algo_vs_raw_cpd_mirrored_no_tiemakers"
    )
