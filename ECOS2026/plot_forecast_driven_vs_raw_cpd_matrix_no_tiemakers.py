...
(excluding tiemaker algorithms).

This version excludes CPD algorithms that always produce ties: CUSUM, EWMA, TwoSample, cpm1B, cpm1S, cpmMS, sbs, wbs.
...
def _build_outcome_frame(best_df: pd.DataFrame, metric: str, rank_mode: str) -> pd.DataFrame:
    ...
    frame["comparison_outcome"] = frame.apply( _classify_outcome, axis=1, raw_col=raw_col, residual_col=residual_col, maximize=maximize, )
...
def _format_metric_label(metric_name: str, dataset_group: str, delta_tag: str) -> str:
    ...
    metric_text = _metric_display_name(metric_name)
    return f"{_math_bold(metric_text)} | {_math_bold(dataset_name)} | delta={delta_tag}"


def _build_counts_table(frame: pd.DataFrame) -> pd.DataFrame:
    rows: List[Dict[str, object]] = []
    grouped = frame.groupby(["cpd_algo", "forecast_algo"], dropna=False)
    for (cpd_algo, forecast_algo), group in grouped: rows.append(
        {
...
def _format_algo_name(algo_name: str) -> str:
    ...
    replacements = {
        ...
        "HybridKNNMLP": "Hybrid\nKNN-MLP",
        "HybridLRXGB": "Hybrid\nLR-XGB",
        "HybridLassoMLP": "Hybrid\nLasso-MLP",
        "HybridLassoXGB": "Hybrid\nLasso-XGB",
    }
...
def _draw_cell(
    ...
    ax.add_patch(rect)
    is_total = is_total_col or is_total_row
    num_weight = "bold" if is_total else "normal"

    lines = [str(n_fd), "---", str(n_tie), "---", str(n_raw)]
    weights = [num_weight, "normal", num_weight, "normal", num_weight]

    # Place text positions as a fraction of cell height so margins are
    # controlled in-cell and shrink/grow with the cell.
    center_x = x + width / 2.0
    margin = min(max(float(text_margin_ratio), 0.0), 0.45)
    y_top = y + height * margin
    y_bottom = y + height * (1.0 - margin)
    y_positions = np.linspace(y_top, y_bottom, 5)
    for (line_txt, line_weight), y_pos in zip(zip(lines, weights), y_positions):
        ax.text(
            center_x,
            y_pos,
            line_txt,
...
def plot_matrix(
    ...
    text_margin_ratio: float = 0.08,
    forecast_label_fontsize: float = 12.0,
    cpd_label_fontsize: float = 12.0,
) -> None:
    ...
    ax.set_xticks(np.arange(n_cols) * cell_width + (cell_width / 2.0))
    ax.set_yticks(np.arange(n_rows) * cell_height + (cell_height / 2.0))
    formatted_forecast_algos = [_format_algo_name(algo) for algo in forecast_algos]
    formatted_cpd_algos = [_format_algo_name(algo) for algo in cpd_algos]
    ax.set_xticklabels(
        formatted_forecast_algos + [r"$\mathbf{Total}$"],
        rotation=50,
        ha="center",
        fontsize=forecast_label_fontsize,
    )
    ax.set_yticklabels(
        formatted_cpd_algos + [r"$\mathbf{Total}$"],
        fontsize=cpd_label_fontsize,
    )
    ax.xaxis.tick_top()
    ax.tick_params(axis="both", length=0)

    # ax.set_title(
        # f"Forecast-Driven vs Raw CPD Matrix\n"
        # f"{metric_label}\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create matrix plots for Forecast-Driven CPD vs raw CPD (excluding tiemakers)"
        )
    parser.add_argument(
        "--metric",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Cell width in data units (text rendering remains unchanged)",
        )
    parser.add_argument(
        "--cell-height",
        type=float,
        default=1.0,
        help="Cell height in data units (text rendering remains unchanged)",
        )
    parser.add_argument(
        "--text-margin-ratio",
        type=float,
        default=0.15,
        help="Top/bottom margin inside each cell as a fraction of cell height (0 to <0.5)",
        )
    parser.add_argument(
        "--forecast-label-fontsize",
        type=float,
        default=12.0,
        help="Font size for forecast algorithm labels (x-axis)",
        )
    parser.add_argument(
        "--cpd-label-fontsize",
        type=float,
        default=12.0,
        help="Font size for CPD algorithm labels (y-axis)",
        )
...
def main() -> int:
    ...
    output_dir = (
        ...
        if args.output_dir
        else script_dir / "results" / "plots" / "forecast_driven_vs_raw_cpd_matrix_no_tiemakers"
        )
    ...
    for metric in metrics_to_run:
        ...
        for dataset_group in dataset_groups:
            ...
            group_outcome_df = _filter_tiemaker_algorithms(group_outcome_df)
            ...
            if args.per_delta:
                ...
                for delta_value in delta_values:
                    ...
                    frame_delta = group_outcome_df[pd.to_numeric(group_outcome_df["delta"], errors="coerce") == float(delta_value) ]
                ...
                plot_matrix(
                    ...
                    text_margin_ratio=args.text_margin_ratio,
                    forecast_label_fontsize=args.forecast_label_fontsize,
                    cpd_label_fontsize=args.cpd_label_fontsize,
                )
                print(f"Metric: {metric} [{_dataset_display_name(dataset_group)}] (delta={delta_tag})")
                print(f"Counts CSV: {counts_csv}")
                print(f"Plot: {plot_path}")
            else:
                delta_tag = _delta_suffix(args.delta)
                group_outcome_df = group_outcome_df[ pd.to_numeric(group_outcome_df["delta"], errors="coerce") == float(args.delta) ]
                if len(group_outcome_df) == 0:
                    continue
                ...
                print(f"Metric: {metric} [{_dataset_display_name(dataset_group)}]")
                print(f"Counts CSV: {counts_csv}")
                print(f"Plot: {plot_path}")
    ...
    if args.with_aggregate:
        for dataset_group in dataset_groups:
            ...
            if args.per_delta:
                ...
                for delta_value in delta_values:
                    delta_tag = _delta_suffix(delta_value)
                    frame_delta = all_outcomes[ pd.to_numeric(all_outcomes["delta"], errors="coerce") == float(delta_value) ]
                    if len(frame_delta) == 0:
                        continue
                    ...
                    delta_tag = _delta_suffix(args.delta)
                    all_outcomes = all_outcomes[ pd.to_numeric(all_outcomes["delta"], errors="coerce") == float(args.delta) ]
