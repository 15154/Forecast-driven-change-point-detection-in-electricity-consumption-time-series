#!/usr/bin/env python3
"""Plot mirrored win/loss bars for Forecast-Driven CPD vs raw CPD.
...
def plot_mirrored_bars(counts_df: pd.DataFrame, metric: str, output_plot: Path) -> None:
    ...
    ax.set_title(
        f"Forecast-Driven vs Raw CPD by Forecast Model ({metric})\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create sorted mirrored bars for Forecast-Driven CPD vs raw CPD wins"
    )
    parser.add_argument(
        "--metric",
        type=str,
...
def main() -> int:
    ...
    output_dir = (
        ...
        if args.output_dir
        else script_dir / "results" / "plots" / "forecast_driven_vs_raw_cpd_mirrored"
    )
        ...
        for dataset_group in dataset_groups:
            ...
            if args.per_delta:
                ...
                for delta_value in delta_values:
                    ...
                    metric_label = f"$\\mathbf{{{metric}}}$ | $\\mathbf{{{dataset_group.upper()}}}$ | delta={delta_tag}"
                    plot_mirrored_bars(
                        counts_df,
                        metric=metric_label,
        ...
        for dataset_group in dataset_groups:
            if args.per_delta:
                ...
                if len(agg_counts_df) == 0:
                    continue
                delta_tag = _delta_suffix(args.delta)
                agg_counts_csv = output_dir / f"forecast_driven_vs_raw_cpd_counts_all_metrics_{dataset_group}_delta_{delta_tag}.csv"
                agg_plot = output_dir / f"forecast_driven_vs_raw_cpd_mirrored_all_metrics_{dataset_group}_delta_{delta_tag}.png"
                agg_counts_df.to_csv(agg_counts_csv, index=False)
                metric_label = f"$\\mathbf{{all_metrics}}$ | $\\mathbf{{{dataset_group.upper()}}}$ | delta={delta_tag}"
                plot_mirrored_bars(
                    agg_counts_df,
