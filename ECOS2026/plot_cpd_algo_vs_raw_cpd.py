#!/usr/bin/env python3
"""Plot mirrored win/loss bars by CPD algorithm.
...
def _build_outcome_frame(best_df: pd.DataFrame, metric: str, rank_mode: str) -> pd.DataFrame:
    ...
    required = {"cpd_algo", raw_col, residual_col}
...
def _counts_from_outcomes(frame: pd.DataFrame, sort_mode: str) -> pd.DataFrame:
    if "cpd_algo" not in frame.columns or "comparison_outcome" not in frame.columns:
        raise ValueError("Expected 'cpd_algo' and 'comparison_outcome' in frame")

    rows = []
    for algo, group in frame.groupby("cpd_algo", dropna=False):
        algo_name = str(algo)
        rows.append(
            {
                "cpd_algo": algo_name,
    ...
    if sort_mode == "alpha":
        counts = counts.sort_values(["cpd_algo"], ascending=[True]).reset_index(drop=True)
    else:
        counts = counts.sort_values(
            ["n_residual_better", "n_raw_better", "cpd_algo"],
...
def build_aggregate_counts(
    ...
    for metric in SUPPORTED_METRICS:
        ...
        if delta_value is not None:
            ...
            if len(frame) == 0:
                continue
        frame = frame[["cpd_algo", "comparison_outcome"]].copy()
        frame["metric"] = metric
        frames.append(frame)

    if len(frames) == 0:
        return pd.DataFrame(columns=["cpd_algo", "n_total", "n_residual_better", "n_raw_better", "n_tie", "n_missing"])
...
def plot_mirrored_bars(counts_df: pd.DataFrame, metric: str, output_plot: Path) -> None:
    ...
    if np.any(y_tie > 0):
        tie_sizes = np.clip(y_tie * 1, 18, 280)
        ax.scatter(
            x,
            np.zeros_like(x),
            s=tie_sizes,
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(counts_df["cpd_algo"].tolist(), rotation=50, ha="right")
    ax.set_ylabel("Number of combinations")
    ax.set_xlabel("CPD algorithm")
    ax.set_title(
        f"Forecast-Driven vs Raw CPD by CPD Algorithm ({metric})\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create mirrored bars by CPD algorithm for Forecast-Driven vs raw CPD"
    )
    parser.add_argument(
        "--metric",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Sort CPD algorithms alphabetically or by residual wins",
    )
    parser.add_argument(
        "--rank-mode",
        type=str,
...
def main() -> int:
    ...
    output_dir = (
        ...
        if args.output_dir
        else script_dir / "results" / "plots" / "cpd_algo_vs_raw_cpd_mirrored"
    )
        ...
        for dataset_group in dataset_groups:
            ...
            if args.per_delta:
                ...
                for delta_value in delta_values:
                    ...
                    counts_csv = output_dir / f"cpd_algo_vs_raw_cpd_counts_{metric}_{dataset_group}_delta_{delta_tag}.csv"
                    plot_path = output_dir / f"cpd_algo_vs_raw_cpd_mirrored_{metric}_{dataset_group}_delta_{delta_tag}.png"
                    counts_df.to_csv(counts_csv, index=False)
                    plot_mirrored_bars(
                        counts_df,
                        metric=f"{metric} | {dataset_group.upper()} | delta={delta_tag}",
                        output_plot=plot_path,
                    )
                    print(f"Input CSV: {best_csv} [{dataset_group.upper()}] (delta={delta_tag})")
                    print(f"CPD algorithms found: {counts_df['cpd_algo'].nunique()}")
                ...
                print(f"Input CSV: {best_csv} [{dataset_group.upper()}]")
                print(f"CPD algorithms found: {counts_df['cpd_algo'].nunique()}")
                print(f"Counts CSV: {counts_csv}")
                print(f"Plot: {plot_path}")
            ...
            if args.per_delta:
                ...
                for delta_value in delta_values:
                    ...
                    if len(agg_counts_df) == 0:
                        continue
                    agg_counts_csv = output_dir / f"cpd_algo_vs_raw_cpd_counts_all_metrics_{dataset_group}_delta_{delta_tag}.csv"
                    agg_plot = output_dir / f"cpd_algo_vs_raw_cpd_mirrored_all_metrics_{dataset_group}_delta_{delta_tag}.png"
                    agg_counts_df.to_csv(agg_counts_csv, index=False)
                    metric_label = f"$\\mathbf{{all_metrics}}$ | $\\mathbf{{{dataset_group.upper()}}}$ | delta={delta_tag}"
                    plot_mirrored_bars(
                        agg_counts_df,
                        metric=metric_label,
